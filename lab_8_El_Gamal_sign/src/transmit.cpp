...
int transmit(int message, data input)
{
    while(true)
    {
        ...
        B = (message % input.divisor_p * fastpow(input.Y, input.K, input.divisor_p)) % input.divisor_p;
        cout << "[TRANSMIT]: A and B solved, transmiting message" << endl;
        //Вычисляем сообщение
        message_res = ((B % input.divisor_p) * fastpow(A,(input.divisor_p - 1 - input.X), input.divisor_p)) % input.divisor_p;
        //--------------------СЕКЦИЯ ДЕБАГА ----------------------------//
